...
function (_ServerEngine) {
  ...
  _createClass(WiggleServerEngine, [{
    ...
    key: "addAI",
    value: function addAI() {// let newAI = new Wiggle(this.gameEngine, null, { position: this.gameEngine.randPos() });
      // newAI.AI = true;
      // newAI.direction = 0;
    ...
    value: function shoot(player) {
      var bullet = new _Bullet.default(this.gameEngine, null, {
        ...
        velocity: new _lanceGg.TwoVector(Math.cos(player.direction) * .15, Math.sin(player.direction) * .15)
      });
      this.gameEngine.addObjectToWorld(bullet); // let wiggle = new Wiggle(this.gameEngine, {}, {
      //     mass: 0.05,
    ...
    value: function stepLogic() {
      ...
      try {
        for (var _iterator = wiggles[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          if (w.AI) {// if (Math.random() < 0.01) w.turnDirection *= -1;
